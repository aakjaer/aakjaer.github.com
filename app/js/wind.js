        ...
        function getGaugeData(url, id) {
            ...
            if(url.match('^http')){
              // assemble the YQL call
                $.getJSON('https://query.yahooapis.com/v1/public/yql?'+'q=select%20*%20from%20html%20where%20url%3D%22'+encodeURIComponent(url)+id+'%22&format=xml&callback=?',
                function(data){
                    if(data.results[0]){
                        var cleanData = filterData(data.results[0]),
                            windData = extractWindData(cleanData);
